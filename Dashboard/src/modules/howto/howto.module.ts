...
export class HowToModule {
    constructor( @Optional() @SkipSelf() parentModule: HowToModule) {
        if (parentModule) {
            throw new Error(`${HowToModule.name} is already loaded. Import it in your application's root module only.`);
        }
    }
}
