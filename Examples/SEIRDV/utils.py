...
def dbpush(job_name, job_type, action_start_dates, action_end_dates, action_names, thedataframe, results, states_data_df):
    ...
    with open('experiment_id.json', 'w') as outfile:
        json.dump(data, outfile)

    # Get COS credentials details
    url = constants.EXPERIMENT_URL+"8af0d08781b365990181b36932040000"
    ...
    print(cosCredentials["apikey"])

    # upload
    with open('experiment_id.json', mode="r") as file:
    ...
    try:
        ...
        with open('experiment_id.json', mode="rb") as file_data:
            cos.Object(cosCredentials['bucketName'],
                        "experiment_id" + "." + "json").upload_fileobj(Fileobj=file_data,
                                                                                    Config=transfer_config)

        # update job with link to output
        experiment_output_payload = {
            "name":  "experiment_id" + "." + "json",
